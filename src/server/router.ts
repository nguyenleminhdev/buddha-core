...
import type { Cb, CbError, Controller, Middleware, MiddlewareConfig } from '../interface'
...
const load_controller = (ROUTER: Router, proceed: Cb) => {
    ...
    waterfall([
        ...
        (cb: CbError) => Promise.all(DATA.path_list.map(async file => {
